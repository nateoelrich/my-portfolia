...
>
	...
	<div class="relative z-10 max-w-4xl px-4 text-center text-white">
		<h1
			id="hero-heading"
			class="mb-6 text-5xl font-bold md:text-7xl {heroVisible ? 'hero-title-in' : 'opacity-0'}"
		>
			Hi, I'm Nate!
		</h1>
		<p class="mb-8 text-2xl font-bold md:text-4xl {heroVisible ? 'hero-subtitle-in' : 'opacity-0'}">
			Let's Build Something Great Together
		</p>
		<nav
			aria-label="Primary call to action"
			class="flex flex-wrap justify-center gap-4 {heroVisible ? 'hero-buttons-in' : 'opacity-0'}"
		>
			<a
				href="/experience"
				class="rounded-lg bg-white px-6 py-3 font-semibold text-purple-600 transition-colors hover:bg-gray-100"
			>
				View My Experience
			</a>
			<a
				href="/projects"
				class="rounded-lg border-2 border-white bg-transparent px-6 py-3 font-semibold text-white transition-all hover:bg-white hover:text-purple-600"
			>
				See Projects
			</a>
		</nav>
...
<style>
	/* Tailwind CSS is used for utility classes; custom styles below.
  I leaned into AI generation for a lot of the CSS here to achieve a cyberpunk aesthetic. 
  I understand what it's doing, curious if it could be improved or optimized, though!
  */
